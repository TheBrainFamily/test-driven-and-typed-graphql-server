...
// interface NewClasssRepository {
//     findAll(): Promise<Class[]>;
// }
// and then change the exported type to the interface instead of the class
// export type ClasssRepositoryType = NewClasssRepository;
...
export class ClasssRepository {
  add(name: string): Promise<Class> {
    const newClass = { id: nextId.toString(), name };
    this.classs.push(newClass);
    nextId++;
    return Promise.resolve(newClass);
  }
  findById(id: string): Promise<Class | null> {
    return Promise.resolve(this.classs.find(i => i.id === id) || null);
  }

  removeById(id: string): Promise<string> {
    this.classs = this.classs.filter(i => i.id !== id);
    return Promise.resolve(id);
  }

  findByName(name: string): Promise<Class | null> {
    return Promise.resolve(this.classs.find(i => i.name === name) || null);
  }
  classs: Class[] = [];
  public findAll(): Promise<Class[]> {
    return Promise.resolve(this.classs);
  }

  removeByName(name: string): Promise<string> {
    const classToRemove = this.classs.find(i => i.name === name) || { id: "0" };
    this.classs = this.classs.filter(i => i.name !== name);
    return Promise.resolve(classToRemove.id);
  }
}

export type ClasssRepositoryType = ClasssRepository;
